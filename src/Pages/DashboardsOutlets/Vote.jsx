...
const Vote = () => {
  ...
  const votingURL = `${window.location.origin}/vote/${contestId}`;
  ...
  const handleVote = async (contestantId) => {
    ...
    try {
      ...
      const response = await axios.post(`${API_URL}/contests/${contestId}/vote`, { contestantId });
      if (response.data.success) {
        const votedContests = JSON.parse(localStorage.getItem('votedContests') || '[]');
      ...
      toast.error(error.response?.data?.error || 'Failed to cast your vote.');
    } finally {
          ...
          <div className="p-6">
            ...
            <div className="flex items-center mb-4">
              <p className="text-gray-700">
                Voting URL: <a href={votingURL} className="text-blue-600 underline">{votingURL}</a>
              </p>
              <CopyToClipboard text={votingURL} onCopy={() => toast.success('URL copied!')}>
                <button className="ml-2 px-4 py-1 bg-blue-500 text-white rounded hover:bg-blue-600 transition-colors duration-200">
                  Copy
                </button>
              </CopyToClipboard>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                {contest.contestants.map((contestant) => (
                  <div key={contestant._id} className="border rounded-lg p-4 hover:shadow-md transition-shadow duration-200">
                    <div className="flex items-center space-x-4">
                      <div className="h-16 w-16 rounded-full overflow-hidden bg-gray-200">
                        {contestant.photoUrl ? (
                          ...
                          <div className="h-full w-full flex items-center justify-center">
                            <span className="text-2xl text-gray-500">{contestant.name?.charAt(0) || '?'}</span>
                          </div>
                        )}
                      </div>
                      ...
                      <button
                        ...
                        className={`px-4 py-2 bg-green-500 text-white rounded hover:bg-green-600 transition-colors duration-200 ${hasVoted || votingInProgress ? 'opacity-50 cursor-not-allowed' : ''}`}
                      >
                        Vote
                      </button>
                    </div>
